...
![The ColorBrewer project: [colorbrewer2.org](https://colorbrewer2.org/)](img/colors/colorbrewer.png){fig-alt="A screnshot of the online tool to test ColorBrewer palettes, which are designed to be used for map purposes. Thus, the tool showcases the colors on a map showing an area in the Southeast of the US." fig-width="70%"}
...
ggplot(bikes, aes(x = season, y = count)) +
  ggforce::geom_sina(
    aes(color = stage(
      day_night, after_scale = clr_lighten(color, .6)
    )),
